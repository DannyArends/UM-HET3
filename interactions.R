...
write.table(lodM, "vita_interactions_2way_males.txt", sep = "\t", quote=FALSE)

#### TODO: split by males & Females
...
for(x in 1:4){
  for(y in 1:4){
    mout[x,y] <- paste0(round(mean(pull.pheno(mcross)[which(m1 == x & m2 == y), "longevity"]),0), 
                 "+/-", round(std(pull.pheno(mcross)[which(m1 ==x & m2 == y), "longevity"]),0))
  }
}
