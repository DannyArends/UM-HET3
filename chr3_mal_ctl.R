...
#
# Mapping longevity on chromosome 3 in control only males, to reproduce the results from:
...
idx <- which(cdata[, "longevity"] > 365 & cdata[, "treatment"] == 0 & cdata[, "sex"] == 1)
...
for(chr in unique(chrs)){
  ...
  points(positions[chrs==chr] + s, lods.m[chrs == chr], t = 'l', lwd=3, col="Blue")
