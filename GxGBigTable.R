  ...
  for(x in 1:(nrow(lodM.m)-1)){
    ...
    for(y in (x+1):nrow(lodM.m)){
      ...
      if(m1 != m2 && lodM.m[x,y] >= 0.0){
        ...
        gtsp <- pull.genoprob(mcross)
        idx <- which(cdata[, "longevity"] >= tp & cdata[, "sex"] == "1")

        cdata <- cdata[idx, ]
        gtsp <- gtsp[idx, ]
        mp1 <- gtsp[, grep(all[rownames(lodM.m)[x]], colnames(gtsp))]
        mp2 <- gtsp[, grep(all[rownames(lodM.m)[y]], colnames(gtsp))]

        lm.alt2 <- lm(longevity ~ site + cohort + treatment + mp1 * mp2 + 0, data = cdata)

        mbigm <- rbind(mbigm, c("M", tp, rownames(lodM.m)[x], rownames(lodM.m)[y], lodM.m[x,y], coef(lm.alt2)))
  ...
  for(x in 1:(nrow(lodM.f)-1)){
    ...
    for(y in (x+1):nrow(lodM.f)){
      ...
      if(m1 != m2 && lodM.f[x,y] >= 0.0){
        ...
        cdata <- cdata[idx, ]
        gtsp <- gtsp[idx, ]
        mp1 <- gtsp[, grep(all[rownames(lodM.f)[x]], colnames(gtsp))]
        mp2 <- gtsp[, grep(all[rownames(lodM.f)[y]], colnames(gtsp))]

        lm.alt2 <- lm(longevity ~ site + cohort + treatment + mp1 * mp2 + 0, data = cdata)

        mbigm <- rbind(mbigm, c("F", tp, rownames(lodM.f)[x], rownames(lodM.f)[y], lodM.f[x,y], coef(lm.alt2)))
