...
pdf(paste0("CTL_MF_24months.pdf"), width = 14, height = 12)
op <- par(cex = 2)
...
write.table(cbind(round(-log10(p.c[[1]]),2), round(res.c[[1]],2)), file = "CTL_BW24_T725_C.txt", sep="\t", quote = FALSE)
write.table(cbind(round(-log10(p.m[[1]]),2), round(res.m[[1]],2)), file = "CTL_BW24_T725_M.txt", sep="\t", quote = FALSE)
write.table(cbind(round(-log10(p.f[[1]]),2), round(res.f[[1]],2)), file = "CTL_BW24_T725_F.txt", sep="\t", quote = FALSE)

setwd("/home/rqdt9/Dropbox (UTHSC GGI)/ITP_HET3_Mapping_Paper_Arends_2021/00_ITP_BioRxiv_All_Key_Files/11_FiguresDanny")
pdf(paste0("CTL_mapping_24mo.pdf"), width = 36, height = 12)
  par(cex=2)
  par(cex.axis=1.5)
  plot(c(0, max(chr.start)), y = c(0, 8), t = 'n', ylab = "LOD", xlab = "Chromosome",xaxt="n", las=2, main = "CTL: Longevity (≥ 725 days) x Bodyweight 24 Months")
  for(x in c(1:19, "X")) {
