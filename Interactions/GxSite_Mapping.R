...
#
# Code to perform progressive mapping of all G x Site effects
#
...
for(x in msequence){
  ...
  for(marker in colnames(pull.geno(mcross))){
    ...
    lm.alt <- lm(longevity ~ sex + site + cohort + treatment + mp + mp:site + 0, data = cdata)
...
write.table(lods.cM, "DataSet/output/combined_GxSite.txt", sep = "\t", quote = FALSE)
...
for(x in msequence){
  ...
  for(marker in colnames(pull.geno(mcross))){
    ...
    lm.alt <- lm(longevity ~ site + cohort + treatment + mp + mp:site + 0, data = cdata)
    n <- sum(!is.na(lm.alt$resid))
    lod <- (n/2) * log10(sum(lm.null$resid^2) / sum(lm.alt$resid^2))
    lods.c <- c(lods.c, lod)
  }
...
write.table(lods.fM, "DataSet/output/female_GxSite.txt", sep = "\t", quote = FALSE)
...
write.table(lods.mM, "DataSet/output/male_GxSite.txt", sep = "\t", quote = FALSE)
