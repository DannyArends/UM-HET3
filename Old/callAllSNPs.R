...
callSNPs <- function(bamfiles) {
  ...
  cmd1 <- paste0(bcftools, " mpileup -q 30 -Ou -a FORMAT/DP -f ", reference, " ", bamstr)
  cmd2 <- paste0(bcftools, " call -mv -Ov ")
  cmd3 <- paste0(bcftools, " view -i '%QUAL>=30 && INFO/DP>10' - -o /halde/wholeGenome/Founders.snps.vcf")
