...
idx <- which(cdata[, "longevity"] > 365 & cdata[, "treatment"] == 0 & cdata[, "sex"] == 0)
...
legend("topright", c("Female controls"), lwd=c(3), col=c("black"), bg="white")
legend("topleft", c("p = 0.05", "p = 0.01"), lwd=1, lty=2, col=c("orange", "green"), bg="white")
box()

marker <- "3_103803615"
mp <- gtsp[, grep(marker, colnames(gtsp))]
lm.alt <- lm(longevity ~ site + cohort + 0, data = cdata)


vioplot(residuals(lm.alt) + mean(cdata[, "longevity"]) ~ apply(mp,1,which.min), xaxt='n', xlab = "GT", main = "CTRL Females at Chr3 top")
...
cdata <- data.frame(longevity = pull.pheno(mcross)[, "longevity"], 
                    ...
                    cohort = as.factor(pull.pheno(mcross)[, "cohort"]), 
                    treatment = as.factor(pull.pheno(mcross)[, "treatment"]))
idx <- which(cdata[, "longevity"] > 365 & cdata[, "treatment"] == 1 & cdata[, "sex"] == 0)
...
axis(1, at = h, unique(chrs))
abline(h = c(4.25, 4.95), col = c("orange", "green"), lty=2)
legend("topright", c("Female NDE"), lwd=c(3), col=c("black"), bg="white")
legend("topleft", c("p = 0.05", "p = 0.01"), lwd=1, lty=2, col=c("orange", "green"), bg="white")
box()
