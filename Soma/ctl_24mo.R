...
#
# Code that Correlated Trait Locus mapping between lifespan and bodyweight at 24 months
...
idx <- which(cdata[, "longevity"] >= 725 & !is.na(cdata[, "bw24"]))
cdata <- cdata[idx,]
gtsp <- gtsp[idx,]
...
confL <- c()
confU <- c()
for(x in seq(730, 1100, 15)){
  male <- which(cdata[, "sex"] %in% 0 & cdata[, "adjLongevity"] > x)
  fema <- which(cdata[, "sex"] %in% 1 & cdata[, "adjLongevity"] > x)
  cMale <- NA; cFema <- NA;

  if(length(male) > 100){
    cMale <- cor(cdata[male, "adjLongevity"], cdata[male, "adjBw24"], use = "pair", method = "spearman");
    confMale <- cor.test(cdata[male, "adjLongevity"], cdata[male, "adjBw24"], use = "pair", method = "pearson", conf.level = 0.50);
  }
  if(length(fema) > 100){
    cFema <- cor(cdata[fema, "adjLongevity"], cdata[fema, "adjBw24"], use = "pair", method = "spearman");
    confFema <- cor.test(cdata[fema, "adjLongevity"], cdata[fema, "adjBw24"], use = "pair", method = "pearson", conf.level = 0.50);
  }
...
pdf(paste0("DataSet/output/CTL_MF_24months.pdf"), width = 14, height = 12)
op <- par(cex = 2)

  plot(c(42, 1100), c(-0.5, 0.2), t = "n", xlab = "Truncation age (days)", ylab = "Correlation BW730 to Tage", 
       main = "Correlation Male/Female Bodyweight 24-months",yaxt = "n", yaxs = "i")

  points(seq(730, 1100, 15), corM[,1], t = "l", col = col.main[1], lwd = 2)
  polygon(c(seq(730, 1100, 15), rev(seq(730, 1100, 15))), c(confL[,1], rev(confU[,1])),col = col.alpha2[1], border=NA)

  points(seq(730, 1100, 15), corM[,2], t = "l", col = col.main[2], lwd = 2)
  polygon(c(seq(730, 1100, 15), rev(seq(730, 1100, 15))), c(confL[,2], rev(confU[,2])),col = col.alpha2[2], border=NA)
  ...
  points(seq(730, 1100, 15), (lods / 50) + -0.5, t = "l", lwd=2)
...
computeDiffCor <- function(mcross, gtsp, cdata, sex = c(0, 1), method = "pearson"){
  ...
  for(marker in colnames(pull.geno(mcross))){
    ...
    if(length(CH) > 100) cCH <- cor(cdata[CH, "adjLongevity"], cdata[CH, "adjBw24"], use = "pair", method = method);
    if(length(BH) > 100) cBH <- cor(cdata[BH, "adjLongevity"], cdata[BH, "adjBw24"], use = "pair", method = method);
    if(length(CD) > 100) cCD <- cor(cdata[CD, "adjLongevity"], cdata[CD, "adjBw24"], use = "pair", method = method);
    if(length(BD) > 100) cBD <- cor(cdata[BD, "adjLongevity"], cdata[BD, "adjBw24"], use = "pair", method = method);
...
write.table(cbind(round(-log10(p.c[[1]]),2), round(res.c[[1]],2)), file = "DataSet/output/CTL_BW24_T725_C.txt", sep="\t", quote = FALSE)
write.table(cbind(round(-log10(p.m[[1]]),2), round(res.m[[1]],2)), file = "DataSet/output/CTL_BW24_T725_M.txt", sep="\t", quote = FALSE)
write.table(cbind(round(-log10(p.f[[1]]),2), round(res.f[[1]],2)), file = "DataSet/output/CTL_BW24_T725_F.txt", sep="\t", quote = FALSE)

pdf(paste0("DataSet/output/CTL_mapping_24mo.pdf"), width = 36, height = 12)
  par(cex=2)
  par(cex.axis=1.5)
  plot(c(0, max(chr.start)), y = c(0, 8), t = 'n', ylab = "LOD", xlab = "Chromosome",xaxt="n", las=2, 
       main = "CTL: Longevity (≥ 725 days) x Bodyweight 24 Months")
  for(x in c(1:19, "X")) {
