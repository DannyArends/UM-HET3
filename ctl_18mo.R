...
#
# Code that Correlated Trait Locus mapping between lifespan and bodyweight at 18 months
...
computeDiffCor <- function(mcross, gtsp, cdata, sex = c(0, 1), method = "pearson"){
  ...
  for(marker in colnames(pull.geno(mcross))){
    ...
    if(length(CH) > 100) cCH <- cor(cdata[CH, "adjLongevity"], cdata[CH, "adjBw18"], use = "pair", method = method);
    if(length(BH) > 100) cBH <- cor(cdata[BH, "adjLongevity"], cdata[BH, "adjBw18"], use = "pair", method = method);
    if(length(CD) > 100) cCD <- cor(cdata[CD, "adjLongevity"], cdata[CD, "adjBw18"], use = "pair", method = method);
    if(length(BD) > 100) cBD <- cor(cdata[BD, "adjLongevity"], cdata[BD, "adjBw18"], use = "pair", method = method);
